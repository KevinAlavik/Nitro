...
void init_display() {
    ...
    for (int y = 0; y < getScreenHeight(); y += LETTER_HEIGHT + TEXT_PADDING) {
        for (int x = 0; x < getScreenWidth(); x += LETTER_WIDTH + TEXT_PADDING) {
            for (int l = 0; l < 256; l++) {
                draw_letter(l, x, y, 255, 255, 255, 1);
                x += LETTER_WIDTH + TEXT_PADDING;
            }
        }
    }
}
